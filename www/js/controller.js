...
gcmPhpControllers.controller('ListController', ['$scope', '$http', function($scope, $http){
    $http.get('http://staging.eimpressive.com/thiru/gcm_php/server/api/news_list.php').success(function(success){
...
gcmPhpControllers.controller('DetailController', ['$scope', '$http', '$routeParams', function($scope, $http, $routeParams){
    $http.get('http://staging.eimpressive.com/thiru/gcm_php/server/api/news_detail.php?id='+$routeParams.id).success(function(success){
        $scope.news = success;
    });
}]);
